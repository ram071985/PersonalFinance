...
public class TransactionServiceTests
{
    ...
    [Test]
    public async Task CreateAsync_WhenOverBudget_Notifies()
    {
        var request = new CreateTransactionRequest
        {
            AccountId = 10,
        ...
        var created = new Transaction { Id = 50, AccountId = 10, Amount = 200m, Description = "Big buy" };
        var full = CreateSampleTransaction(50);
        full.Amount = 200m;
        full.CategoryId = 5;
        full.Type = TransactionType.Expense;

        _repo.Setup(r => r.AddAsync(It.IsAny<Transaction>())).ReturnsAsync(created);
        _repo.Setup(r => r.GetByIdAsync(50)).ReturnsAsync(full);
        _budgets.Setup(b => b.GetByMonthAsync(It.IsAny<int>(), It.IsAny<int>()))
            .ReturnsAsync(new List<PersonalFinance.Core.Dtos.Budgets.BudgetDto>
            {
                new()
                {
                    Id = 1,
        ...
        _currentUser.Setup(c => c.Email).Returns("u@test.local");

        await _sut.CreateAsync(request);

        _notifications.Verify(n => n.NotifyBudgetExceededAsync(
            "test-user",
            "u@test.local",
            "Food",
            150m,
